...
def engineer_features(df,training=True,date_param=date(2019,7,30)):
    """
    ...
    By defult, the target revenue is calculated for the 30/07/2019 (one month after the last date: 30/06/2019)
    ...
    for d,day in enumerate(dates):
        ...
        '''
        ## get the target revenue (next month)
        ...
        ## get the target revenue (date entered as an parameter)
       
        date_param = np.datetime64(date_param, 'D') 
        #print("date_param: {}".format(date_param))
        #print("current: {}".format(current))
        #diff_current_date_param = date_param - current
        #print("diff_current_date_param: {}".format(diff_current_date_param))
        #plus_x = current + np.timedelta64(diff_current_date_param,'D')
        #print("plus_x: {}".format(plus_x))
        mask = np.in1d(dates, np.arange(current,date_param,dtype='datetime64[D]'))
        y[d] = df[mask]['revenue'].sum()
        
        '''
        ## get the target revenue (next 45 days)   
        plus_45 = current + np.timedelta64(45,'D')
        mask = np.in1d(dates, np.arange(current,plus_45,dtype='datetime64[D]'))
        eng_features['45_days_target_revenue'] = df[mask]['revenue'].sum()
        
        ## get the target revenue (next 60 days) 
        plus_60 = current + np.timedelta64(60,'D')
        mask = np.in1d(dates, np.arange(current,plus_60,dtype='datetime64[D]'))
        eng_features['2_months_target_revenue'] = df[mask]['revenue'].sum()
        '''
        ...
        stop_date = date_param - np.timedelta64(365,'D')
